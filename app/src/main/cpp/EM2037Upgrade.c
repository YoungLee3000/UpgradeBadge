...
JNIEXPORT jint JNICALL Java_com_newland_core_Native_ImportBinFile
...
int EM2037_Upgrade_PackCmd(char* in, int inlen, char* out)
{
    ...
    outlen += inlen;
    CalculateCrc32(out, outlen, crc, &nCRC);
    out[outlen++] = crc[0];
    out[outlen++] = crc[1];
    out[outlen++] = crc[2];
    out[outlen++] = crc[3];
...
JNIEXPORT void JNICALL Java_com_newland_core_Native_SetFrameSize
  ...
  (JNIEnv * env, jobject obj, jstring jFileName)
{
	...
	m_nApplPackIdx = -1;
	m_nImportStatus = -1;
	const char* strFileName = (*env)->GetStringUTFChars(env, jFileName, false);
	do
	{
		if(strFileName == NULL)
		{
		...
		memcpy(&Pack_All_Info, &m_lvfilebuf[m_nFilelen-sizeof(TuIMGPack_All)], sizeof(TuIMGPack_All));
	...
	if (strFileName)
	{
		(*env)->ReleaseStringUTFChars(env, jFileName, strFileName);
	}
	return m_nImportStatus;
}

JNIEXPORT jstring JNICALL Java_com_newland_core_Native_GetDevNameFromImportFile
  (JNIEnv * env, jobject obj)
{
	...
	TuIMGPack_All * pPack_All_Info = (TuIMGPack_All *)&m_lvfilebuf[m_nFilelen-sizeof(TuIMGPack_All)];
	return (*env)->NewStringUTF(env, pPack_All_Info->HardName);
}

JNIEXPORT jstring JNICALL Java_com_newland_core_Native_GetVersionFromImportFile
        (JNIEnv * env, jobject obj)
{
    if ( (NULL == m_lvfilebuf) || (0 != m_nImportStatus) )
    {
    ...
    TuIMGPack_All * pPack_All_Info = (TuIMGPack_All *)&m_lvfilebuf[m_nFilelen-sizeof(TuIMGPack_All)];
    return (*env)->NewStringUTF(env, pPack_All_Info->Version);
}
...
JNIEXPORT jint JNICALL Java_com_newland_core_Native_GetParam
  (JNIEnv * env, jobject obj, jint nUpgradeType, jobject pImportFileStatus, jobject pFrameSize, jobject pCmdPackCnt, jobject pDataPackCnt)
{
	if (NULL == m_lvfilebuf)
	{
		return -1;
	...
	(*env)->SetIntField(env, pImportFileStatus, id, m_nImportStatus);
	...
	if (0 == m_nImportStatus)
	{
		...
		(*env)->SetIntField(env, pFrameSize, id, c_FrameSize);
		(*env)->SetIntField(env, pCmdPackCnt, id, 5);
		(*env)->SetIntField(env, pDataPackCnt, id, Frame);
	}
	else
	{
		(*env)->SetIntField(env, pFrameSize, id, 0);
		(*env)->SetIntField(env, pCmdPackCnt, id, 0);
		(*env)->SetIntField(env, pDataPackCnt, id, 0);
	}
	return 0;
}

JNIEXPORT jbyteArray JNICALL Java_com_newland_core_Native_GetPackCmd
  (JNIEnv * env, jobject obj, jint nUpgradeType, jint nPackIdx)
{
	...
	switch (nPackIdx)
	{
		...
		//发送次数：  "!Frames:XX" （等于 (DataLens + nFrameSize - 1) / nFrameSize ）	
			sprintf(lvHeadBuf, "!Frames:%d",(pPack_All_Info->block[nTypeIdx].Lens + c_FrameSize - 1)/(c_FrameSize));
			break;
		case 3:
		//"!FileType:kern"
			if (UPGRADE_TYPE_BOOT == nUpgradeType)
	...
	jbyteArray array = (*env)->NewByteArray(env, nLenRet);
	(*env)->SetByteArrayRegion(env, array, 0, nLenRet, lvPackCmdBuf);
	return array;
}


JNIEXPORT jbyteArray JNICALL Java_com_newland_core_Native_GetPackData
  (JNIEnv * env, jobject obj, jint nUpgradeType, jint nPackIdx)
{
	if (NULL == m_lvfilebuf)
	{
	...
	int nLenRet = EM2037_Upgrade_PackData(lvPackDataBufin, c_FrameSize,lvPackDataBufout);
	if (nLenRet < 1)
	{
		return NULL;
	}
	jbyteArray array = (*env)->NewByteArray(env, nLenRet);
	(*env)->SetByteArrayRegion(env, array, 0, nLenRet, lvPackDataBufout);
	return array;
}
